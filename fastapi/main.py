...
def submit_section_answers(data: SectionSubmitPayload, db_conn=Depends(get_db_conn)):
    """
    Submit all answers for a single category/section.
    This endpoint saves all answers for a category in one request.
    """
    try:
        with db_conn as conn:
            cur = conn.cursor(cursor_factory=RealDictCursor)
            ...
            for answer in data.answers:
                ...
                cur.execute("""
                    SELECT id FROM assessment_answers
                    WHERE employee_id=%s AND band=%s AND question=%s;
                """, (data.employee_id, data.band, answer.question))
                ...
                if existing:
                    ...
                    cur.execute("""
                        ...
                        SET answer_value=%s, updated_at=NOW()
                        WHERE id=%s;
                    """, (answer.answer_value, existing["id"]))
                else:
                    # INSERT new answer
                    cur.execute("""
                        INSERT INTO assessment_answers 
